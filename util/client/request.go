...
func closeWithErrorAndLog(pw *io.PipeWriter, err error) {
	newErr := pw.CloseWithError(err)
	log.Println(err)
	if newErr != nil {
		log.Println("err on pw.CloseWithError", newErr)
	}
}
...
func (r *request) SetPathParam(name, value string) error {
	if r.pathParams == nil {
